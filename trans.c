...
static void cool_trans(size_t M, size_t N, double A[N][M], double B[M][N],
                       double tmp[TMPCOUNT]) {

    // we access by blocks
    for (size_t i = 0; i < N; i = i + block_size) {
        for (size_t j = 0; j < M; j = j + block_size) {

            // now we execute the transpose for a block
            for (size_t block_i = i; block_i < i + block_size; block_i++) {
                for (size_t block_j = j; block_j < j + block_size; block_j++) {

                    // do transpose for non-diagonals of bigger matrix
                    if (block_i != block_j) {
                        B[block_j][block_i] = A[block_i][block_j];
                    }
                }
                if (i == j) {
                    B[block_i][block_i] = A[block_i][block_i];
                }
            }
        }
    }
